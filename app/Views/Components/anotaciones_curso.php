        ...
        <?php foreach ($estudiantes as $estudiante): ?>
            ...
            <div class="modal fade" id="crearAnotacionesModal<?= $estudiante['estudiante_id'] ?>" tabindex="-1" aria-labelledby="crearAnotacionesModalLabel<?= $estudiante['estudiante_id'] ?>" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="crearAnotacionesModalLabel<?= $estudiante['estudiante_id'] ?>">Crear Anotación para <?= $estudiante['nombre_estudiante'] ?></h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <form action="<?= site_url('anotaciones/crear') ?>" method="post">
                                ...
                                <!-- Grado de la Anotación -->
                                <div class="mb-3">
                                    <label for="grado" class="form-label">Grado de la Anotación:</label>
                                    <select class="form-select" name="grado" required>
                                        <option value="connotado">Connotación negativa</option>
                                        <option value="grave">Grave</option>
                                        <option value="gravisimo">Gravísimo</option>
                                        <option value="extremo">Extremo</option>
                                        <option value="connotado">Connotación positiva</option>
                                        <option value="bueno">Bueno</option>
                                        <option value="buenisimo">Buenísimo</option>
                                        <option value="felicitaciones">Felicitaciones</option>
                                    </select>
                                </div>

                                <button type="submit" class="btn btn-primary">
                                    <i class="bi bi-check-circle-fill"></i> Agregar Anotación
                                </button>
                            </form>
                        </div>
                    </div>
